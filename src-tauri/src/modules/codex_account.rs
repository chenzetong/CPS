...
async fn perform_managed_token_refresh(
    ...
    match running_codex_oauth_account_ids() {
        Ok(account_ids) if account_ids.contains(&account.id) => {
            ...
            return Err(
                "官方 ChatGPT/Codex 客户端正在使用此账号；为避免重复轮换 refresh_token，CPS 已暂停该账号刷新。请先关闭官方客户端，或等待官方客户端写回新凭据后重试。"
                    .to_string(),
            );
        }
        Ok(_) => {}
...
mod tests {
    use super::{
        ...
        managed_account_runtime_tokens_need_refresh, merge_existing_auth_file_value,
        normalize_account_proxy_url, now_timestamp, parse_agent_identity_from_value,
        ...
        sync_current_live_oauth_snapshot_before_switch, sync_managed_projection_from_auth_dir,
