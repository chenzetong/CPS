...
- **CPS 已对齐上游 v1.3.25**，同时保留 CPS 品牌与更新通道、多 SSH 主机 Codex 账号及历史同步、远端项目恢复、孤立 rollout 恢复和安全的 app-server 生命周期处理。
- **上游新增的授权进度、实例账号占用保护和 direct app-server 清理已与 CPS 的远端启动上下文恢复、按 profile 清理孤立 app-server 逻辑合并。**
...
- **CPS 已对齐上游 v1.3.15，同时保留事务式 SSH 账号/历史同步、孤立 rollout 恢复、远程项目恢复、app-server 安全重载、CPS 品牌和 fork 自有更新通道。**
...
- **SSH Codex 账号同步改为事务式执行**：CPS 先把完整账号投影上传到远端暂存目录并校验，再暂停运行中的 app-server，将鉴权包、会话数据库和 rollout 修复作为一次可恢复操作执行。
- **远端 rollout 对账**：同时扫描活动与归档 rollout 目录，按真实会话内容恢复缺失或失效的 `rollout_path`、`has_user_event` 和 `cwd`；仅报告没有数据库记录的孤立 rollout，不凭空插入 thread 行。

### 变更

- **统一恢复备份**现在包含清单、哈希、权限、时间戳、计划操作、SQLite 在线备份、旧账号投影以及所有待替换 rollout。
- **版本号恢复到与原仓库对应的 `0.x` 序列**。CPS `0.26.6` 基于上游 `v1.3.10`，并包含此前 CPS `1.3.11` 已提供的历史恢复改动。

### 修复

- **SSH 同步中断不再留下混合鉴权状态**：失败时恢复全部旧投影文件、删除同步前不存在的新文件，并在返回错误前核对恢复后的哈希。
- **使用乐观指纹保护并发 rollout 追加**：文件变化时重新规划一次，连续变化则在替换前中止。
- **含真实用户事件的远程对话在重启后保持可见**，即使 `first_user_message` 为空或 SQLite 中的 rollout 路径已经失效。

---
## CPS SSH 历史功能奠基版本 - 2026-07-21

### 新增

- **会话管理器可显示远程 SSH Codex 历史**：CPS 以只读方式读取已配置 SSH 主机上的可见用户会话，并按远程工作目录分组展示。
- **SSH 账号同步可修复远端 Codex 会话状态**：同步时会先备份，再修复 provider 元数据、用户事件可见性和 rollout 元数据，校验通过后重载远端 app-server。

### 变更

- **fork 版本正式使用 CPS 品牌**：macOS Bundle、应用窗口、运行时文案、更新地址和 Release 资产统一使用 CPS，并指向 `chenzetong/CPS` 仓库。
- **同步上游 v1.3.10**，同时保留 SSH 会话修复功能。

### 修复

- **Codex 重启后保留远程项目**：重启恢复现在会同时写回权威的 `selected-project`、旧版远程项目字段和可达主机自动连接设置。
...
- **CPS 已对齐上游 v1.3.14，同时保留事务式 SSH 账号/历史同步、孤立 rollout 恢复、远程项目恢复和 app-server 安全重载。**
- **本版本不包含 CPS API 服务的 SSH 自动反向隧道**：实验性实现 `1887a31b` 已撤销；如需让远端主机访问 API 服务，可改用手动维护的 reverse tunnel。
...
- **CPS 已对齐上游 v1.3.11，同时保留事务式 SSH 历史同步、孤立 rollout 恢复、远程项目恢复和 app-server 安全重载。**
- **上游 AppHandle 生命周期、CLI 终端启动选项、会话亲和与 Agent Identity 支持已和 CPS 远程启动上下文组合，不会覆盖原有远程恢复逻辑。**
- **仓库关闭 Issues 时，定时上游同步冲突仍会安全报告**：冲突路径写入 Actions Summary 和逐文件错误注解，不会因创建 Issue 失败再叠加第二个工作流错误。
