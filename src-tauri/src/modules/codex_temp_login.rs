...
mod tests {
    ...
    fn auth_hook_node_options_normalizes_windows_path_separators() {
        ...
        let options = build_auth_hook_node_options(Path::new(
            r"C:\Users\test\profile space\.antigravity_cockpit\codex-temp-login\abc\.cockpit-auth-hook.cjs",
        ));
        assert_eq!(
            options,
            "--require=\"C:/Users/test/profile space/.antigravity_cockpit/codex-temp-login/abc/.cockpit-auth-hook.cjs\""
        );
