...
### 修复
- 恢复最初的 CPS 更新公钥。v1.3.50 中的一处字符抄写错误被 v1.3.59 继承，导致在线更新出现 `The signature verification failed`。
- 发布前验证签名密钥与应用公钥一致，并在上传和发布收尾前对所有更新包执行密码学签名验证。

### 升级说明
- v1.3.50、v1.3.59 用户需要从本次 Release 下载安装包，覆盖安装一次。这些版本内置了错误的更新公钥，无法通过应用内更新修复。保留原有应用数据即可；后续自动更新继续使用最初的 CPS 签名密钥。

## [1.3.59] - 2026-09-23

### CPS 集成
- 合并上游 v1.3.51–v1.3.59，保留 CPS 品牌、更新签名配置、账号独立代理与 SSH 历史支持；新的会话位置选择器排除只读 SSH 位置。
