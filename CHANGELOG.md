...
### CPS integration
- Merge upstream v1.3.51–v1.3.59 while preserving CPS branding, updater signing configuration, per-account proxies, and SSH history support. The new session location picker excludes read-only SSH locations.
...
- **CPS is aligned with upstream v1.3.15 while retaining transactional SSH account/history synchronization, orphan rollout recovery, remote-project restoration, safe app-server reloads, CPS branding, and the fork-owned updater channel.**
