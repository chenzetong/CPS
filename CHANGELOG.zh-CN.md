...
### 修复
- **修复 Codex 本地 API 服务和 `image_generation` 设置持久化问题**：保存后的本地访问运行选项在刷新、切号和 API 服务启动后保持稳定。
- **修复 Codex Token 刷新跨进程并发问题**：托管 Token 刷新改用跨进程文件锁，并在其他进程已推进 token 代际后复用刷新结果，减少重复刷新。
...
### 新增
- **Codex 账号备注支持敏感信息**：账号备注可保存 2FA 秘钥、密码、手机号和其他备注，并可在账号卡片与操作弹框中查看 2FA 动态验证码。
- **Codex 批量导入和删除更适合长任务**：批量导入支持任务恢复，批量删除支持进度、暂停、继续、失败重试和清理。
- **Codex 数据迁移支持会话记录**：账号迁移可导出和导入 Codex 会话，并在导入时跳过已存在会话和提示缺失目标实例。
- **Codex 本地 API 服务新增单账号路由和历史估值重算**：可固定使用首个账号处理请求，并可按当前模型价格重算历史请求日志估值。
- **Codex 会话可见性修复支持预览**：修复前可先预览将要变更的实例和会话记录，再确认执行修复。

### 变更
- **Codex 模型供应商列表识别更兼容**：可解析更多常见模型列表返回格式，并保留模型显示名称。
- **仪表盘单个平台卡片不再撑满整行**：只显示一个平台时仍保持半行宽度，窄屏下再回退为单列。
- **多个平台账号页批量选择更一致**：Antigravity、Cursor、Gemini、GitHub Copilot、Kiro、Qoder、Trae、Windsurf、Zed 和 CodeBuddy 系列账号页使用统一的批量选择工具栏。
- **平台包操作区更紧凑**：减少主工具栏重复操作，将卸载等辅助操作收进更多菜单，降低窄窗口按钮挤压。

### 修复
- **修复 Codex 配置保存时可能重置字段的问题**：保存账号备注、API Key 配置或本地网关选项时，不再误重置供应商、Base URL、模型目录或 `image_generation` 设置。
- **修复宿主调用本地平台 adapter 可能误走系统代理的问题**：本地 adapter RPC 会绕过代理，并保留更完整的底层网络错误信息，便于定位 OAuth、账号刷新或本地 RPC 失败。
- **改进 Cursor 账号刷新和 OAuth 错误提示**：失败时会展示更完整的底层网络错误链路。
- **修复顶部横幅可见性设置不会立即同步到当前窗口的问题**。
